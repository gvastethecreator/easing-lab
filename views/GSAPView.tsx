...
export const GSAPView: React.FC<GSAPViewProps> = ({ 
    customEaseId, points, setPoints, progressRef, duration, setDuration, range, setRange 
}) => {
    ...
    const functionsToDisplay = useMemo(() => {
        ...
        return GSAP_EASING_FUNCTIONS.filter(func => func.category === activeCategory).sort((a,b) => a.name.localeCompare(b.name));
    }, [activeCategory, customGSAPEasing]);
    
    const categoryItems = useMemo(() => (Object.values(GSAPEasingCategory) as GSAPEasingCategory[]).map(c => ({ label: c, value: c })), []);
