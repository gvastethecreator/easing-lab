...
export const CubicBezierView: React.FC<CubicBezierViewProps> = ({ 
    ...
    const filteredFunctions = useMemo(() => {
      return EASING_FUNCTIONS.filter(func => {
        const categoryMatch = activeCategory === EasingCategory.ALL || func.category === activeCategory;
        const typeMatch = activeType === EasingType.ALL || func.type === activeType;
        const isOtherAndFiltered = func.type === EasingType.OTHER && activeType !== EasingType.ALL && activeType !== EasingType.OTHER;
    ...
    const categoryItems = useMemo(() => (Object.values(EasingCategory) as EasingCategory[]).map(c => ({ label: c, value: c })), []);
    const typeItems = useMemo(() => (Object.values(EasingType) as EasingType[]).map(t => ({ label: t, value: t })), []);
