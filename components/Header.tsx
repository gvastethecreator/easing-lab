...
export const Header: React.FC<HeaderProps> = ({ activeView, setView, progressRef, isPlaying, togglePlay }) => {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            gsap.set([follower, ...trail], { 
                xPercent: -50, yPercent: -50, scale: 0, autoAlpha: 0, x: standbyPos.x, y: standbyPos.y 
            });
            ...
            const trailXTo = trail.map((p, i) => gsap.quickTo(p, "x", { duration: 0.6 + (i + 1) * 0.05, ease: "power3.out" }));
            const trailYTo = trail.map((p, i) => gsap.quickTo(p, "y", { duration: 0.6 + (i + 1) * 0.05, ease: "power3.out" }));
            ...
            >
                <div className="flex items-center justify-between h-16 relative z-20">
                    <div className="flex items-center gap-6">
                        <div className="flex flex-col">
                            ...
                            <span className="text-[10px] font-mono text-text-secondary uppercase tracking-widest opacity-60 -mt-1 hidden sm:block">Motion Library</span>
                        </div>
                        ...
                        <nav className="hidden md:flex items-center gap-1 p-1 bg-surface-2/50 dark:bg-surface-1/30 rounded-xl border border-border-subtle">
                            <NavButton label="Cubic Bezier" isActive={activeView === 'cubic'} onClick={() => setView('cubic')} />
                            <NavButton label="GSAP Gallery" isActive={activeView === 'gsap'} onClick={() => setView('gsap')} />
                        </nav>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div className="md:hidden relative">
                            ...
                            >
                                ...
                                <ChevronDownIcon className={`transition-transform duration-200 ${mobileMenuOpen ? 'rotate-180' : ''}`} />
                            </button>
                            ...
                            {mobileMenuOpen && (
                                <>
                                    ...
                                    <div className="absolute top-full right-0 mt-2 w-32 bg-surface-1 dark:bg-surface-2 border border-border-subtle rounded-xl shadow-xl z-20 overflow-hidden flex flex-col p-1 animate-in fade-in zoom-in-95 duration-200">
                                        <button 
                                            onClick={() => { setView('cubic'); setMobileMenuOpen(false); }}
                                        ...
                                        <button 
                                            onClick={() => { setView('gsap'); setMobileMenuOpen(false); }}
                ...
                <div className="absolute inset-0 pointer-events-none z-0 rounded-2xl overflow-hidden">
                    <div 
                        ...
                        style={{ background: 'radial-gradient(circle, var(--accent-primary) 0%, transparent 70%)', opacity: 0 }}
                    />
