...
const PreviewButton: React.FC<{ label: string; isActive: boolean; onClick: () => void; }> = ({ label, isActive, onClick }) => (
    <button 
        onClick={onClick} 
...
export const AnimationPreview: React.FC<AnimationPreviewProps> = ({ ease, duration, range, progressRef }) => {
    ...
    ctxRef.current = gsap.context(() => {
        ...
        const getProps = (isGhost: boolean) => {
            ...
            switch (activeAnimation) {
                case 'Move':
                case 'Stagger': return { xPercent: 350 * range, ease: easeVal, opacity: alpha };
                case 'Scale': return { scale: 1 + 1 * range, ease: easeVal, opacity: alpha };
                case 'Rotate': return { rotation: 180 * range, ease: easeVal, opacity: alpha };
                default: return {};
            }
        };
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <span className="text-[10px] font-bold text-text-secondary uppercase tracking-wider">Preview</span>
        <div className="flex gap-2 items-center flex-wrap">
            ...
            <div className="flex gap-1">
                {(['Move', 'Scale', 'Rotate', 'Stagger'] as AnimationType[]).map(type => (
                    <PreviewButton key={type} label={type} isActive={activeAnimation === type} onClick={() => setActiveAnimation(type)} />
                ))}
            </div>
        </div>
      </div>
      ...
      <div className="w-full bg-surface-2/30 border border-border-subtle rounded-xl flex flex-col relative overflow-hidden">
        <div className="h-28 w-full flex items-center px-6 relative">
            <div className="absolute inset-0 opacity-20 pointer-events-none" 
                style={{ backgroundImage: 'linear-gradient(90deg, var(--border-subtle) 1px, transparent 1px)', backgroundSize: '40px 100%' }}>
            </div>

            {activeAnimation === 'Stagger' ? (
                <div className="flex flex-col gap-2 w-full">
                    {[0, 1, 2].map((i) => (
                        <div key={i} ref={el => { targetsRef.current[i] = el; }} className="w-6 h-6 rounded bg-accent-primary shadow-sm relative z-10" />
                    ))}
                </div>
            ) : (
                <>
                    <div ref={el => { targetsRef.current[1] = el; }} className="w-10 h-10 rounded-lg bg-text-secondary/20 absolute left-6 pointer-events-none" />
                    {activeAnimation === 'Move' && [...Array(5)].map((_, i) => (
                        <div key={`trail-${i}`} ref={el => { trailRefs.current[i] = el; }} className="w-10 h-10 rounded-lg bg-accent-primary/30 absolute left-6 pointer-events-none mix-blend-multiply dark:mix-blend-screen z-0 will-change-transform" />
                    ))}
                    <div ref={el => { targetsRef.current[0] = el; }} className="w-10 h-10 rounded-lg bg-accent-primary shadow-lg relative z-10 flex items-center justify-center will-change-transform">
        ...
        <div className="h-1 w-full bg-surface-2 relative">
            <div ref={progressBarRef} className="h-full bg-accent-primary origin-left will-change-transform" style={{ transform: 'scaleX(0)' }} />
        </div>
      </div>
