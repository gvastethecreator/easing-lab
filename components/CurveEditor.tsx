...
export const CurveEditor: React.FC<CurveEditorProps> = ({ p1, setP1, p2, setP2, duration, setDuration, range, setRange, progressRef }) => {
  ...
  const { state: coords, set: setCoords, undo, redo, reset, canUndo, canRedo } = useHistory(initialState);
  ...
  const updateParent = useCallback((newCoords: { p1: Point, p2: Point }) => {
      setP1(newCoords.p1);
      setP2(newCoords.p2);
  }, [setP1, setP2]);
  ...
  useEffect(() => {
    ...
    const onTick = () => {
        ...
        try {
            ...
            if (rawPath) {
                const pointOnPath = MotionPathPlugin.getPositionOnPath(rawPath, progressRef.current.progress);
  ...
  const applyPreset = (type: 'linear' | 'easeIn' | 'easeOut' | 'easeInOut') => {
      ...
      switch(type) {
          case 'linear': newState = { p1: { x: 0, y: 0 }, p2: { x: 1, y: 1 } }; break;
          case 'easeIn': newState = { p1: { x: 0.42, y: 0 }, p2: { x: 1, y: 1 } }; break;
          case 'easeOut': newState = { p1: { x: 0, y: 0 }, p2: { x: 0.58, y: 1 } }; break;
          case 'easeInOut': newState = { p1: { x: 0.42, y: 0 }, p2: { x: 0.58, y: 1 } }; break;
      }
  ...
  if (dragStartCoords) {
      const gCp1 = { x: dragStartCoords.p1.x * viewBoxSize, y: viewBoxSize - dragStartCoords.p1.y * viewBoxSize };
      const gCp2 = { x: dragStartCoords.p2.x * viewBoxSize, y: viewBoxSize - dragStartCoords.p2.y * viewBoxSize };
  ...
  const toolbarActions = (
    <>
        ...
        <div className="w-px h-5 bg-border-subtle mx-2 self-center"></div>
        <button onClick={() => applyPreset('linear')} className="p-1.5 rounded-md hover:bg-surface-2 text-text-secondary transition-colors" title="Reset to Linear"><ResetIcon /></button>
    </>
  );

  const canvas = (
    <div className="aspect-square w-full max-w-[280px] relative">
        <svg ref={svgRef} viewBox={`-40 -80 ${viewBoxSize + 80} ${viewBoxSize + 160}`} className="overflow-visible w-full h-full touch-none">
            <GraphGrid size={viewBoxSize} />
            
            {/* Ghost Trace (Original Curve before Drag) */}
            {ghostPathData && (
                <path d={ghostPathData} stroke="var(--text-placeholder)" strokeWidth="2" strokeDasharray="4 4" fill="none" className="pointer-events-none opacity-50" />
            )}

            {/* Connecting Lines */}
            <line x1={0} y1={viewBoxSize} x2={cp1.x} y2={cp1.y} stroke="var(--accent-primary)" strokeWidth="2" strokeDasharray="4" opacity="0.4" className="pointer-events-none" />
            <line x1={viewBoxSize} y1={0} x2={cp2.x} y2={cp2.y} stroke="var(--accent-primary)" strokeWidth="2" strokeDasharray="4" opacity="0.4" className="pointer-events-none" />
            
            {/* Curve */}
            <path ref={pathRef} d={pathData} stroke="currentColor" strokeWidth="4" fill="none" className="text-text-primary drop-shadow-md pointer-events-none" strokeLinecap="round" />
            
            {/* Marker */}
            <circle ref={markerRef} r="8" fill="var(--accent-primary)" stroke="white" strokeWidth="2" className="shadow-sm pointer-events-none" />
      ...
      <>
        ...
        <div className="flex gap-6 pt-4 border-t border-border-subtle">
                ...
                <input
                    ...
                    value={range} onChange={(e) => setRange(parseFloat(e.target.value))}
                    className="w-full h-1.5 bg-surface-2 rounded-lg appearance-none cursor-pointer accent-accent-primary hover:accent-accent-primary-hover"
                />
            </div>
        </div>
  ...
  return (
    <EditorLayout
        ...
        canvas={canvas}
        controls={controls}
        preview={<AnimationPreview ease={bezierString} duration={duration} range={range} progressRef={progressRef} />}
