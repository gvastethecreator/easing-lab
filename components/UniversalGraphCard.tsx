...
export const UniversalGraphCard: React.FC<UniversalGraphCardProps> = React.memo(({
  ...
  const grid = useMemo(() => (
      <GraphGrid size={224} subdivisions={4} showMidLines={false} opacity={0.3} showFineGrid={false} />
  ), []);
  ...
  const handleMouseEnter = () => {
    if (ctxRef.current) ctxRef.current.revert();
    if (!cardRef.current || !markerRef.current || !pathRef.current) return;
  ...
  const handleMouseLeave = () => {
    if (!cardRef.current || !pathRef.current || !markerRef.current) {
      return;
    }

    gsap.killTweensOf([cardRef.current, pathRef.current, markerRef.current, tooltipRef.current].filter(Boolean));
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Feedback Copied */}
        <div className={`absolute inset-0 z-30 flex items-center justify-center pointer-events-none transition-all duration-300 ${copied ? 'opacity-100 backdrop-blur-[2px]' : 'opacity-0'}`}>
            <div className={`transform transition-all duration-300 ${copied ? 'translate-y-0 scale-100' : 'translate-y-4 scale-90'}`}>
      ...
      <div className="text-center px-1">
        ...
        {subtitle && <p className="text-[10px] font-mono text-text-secondary truncate mt-0.5 opacity-80">{subtitle}</p>}
      </div>
    </div>
  );
});
